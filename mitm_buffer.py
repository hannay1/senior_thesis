...
class MITM_Interface:
	...
	def read_traffic(self, account, password):
		...
		tup = str(self.assign_strength(password))
		strength = tup[0]
		edit_count = tup[1]
		transitions = tup[2]
		length = str(len(password))
		self.pw_db_router.insert_into_table_S(self.current_user, account, password)
		number_shared_passwords = self.pw_db_router.get_user_pwords_accounts(self.current_user)
		...
		self.pw_db_router.insert_into_table_A(self.current_user, p_id, account, length, strength, edit_count, transitions)
		self.pw_db_router.insert_into_table_B(self.current_user, password_id, num_passwords, associated_accounts)
		pass
